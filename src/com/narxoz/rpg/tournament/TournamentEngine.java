...
public class TournamentEngine {
    ...
    public TournamentResult runTournament() {
        ...
        while (hero.isAlive() && opponent.isAlive() && round < maxRounds) {
            ...
            result.addLine("Hero planning actions:");
            ...
            if (opponent.isAlive()) {
                result.addLine("Enemy attacking on " + opponent.getAttackPower() + " damage!");
                dodge.handle(opponent.getAttackPower(), hero);
            }

            result.addLine(String.format("[Round result %d] Enemy HP: %d | Hero HP: %d",
                    round, opponent.getHealth(), hero.getHealth()));
        }
